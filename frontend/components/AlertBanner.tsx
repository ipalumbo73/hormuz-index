    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-mono text-[11px] font-semibold px-2 py-0.5 rounded-[10px]" style={{ background: 'rgba(239,68,68,0.2)', color: '#ef4444' }}>
            {alerts.length} alert attiv{alerts.length === 1 ? 'o' : 'i'}
          </span>
          <span className="text-[13px] text-white/70">{alerts[0].title}: {alerts[0].message}</span>
        </div>
        <span className="text-white/30 text-xs transition-transform" style={{ transform: expanded ? 'rotate(180deg)' : 'rotate(0)' }}>▼</span>
      </div>
      {expanded && (
