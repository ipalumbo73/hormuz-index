    ...
    <header className="sticky top-0 z-50 border-b border-white/[0.06] backdrop-blur-xl" style={{ background: 'linear-gradient(180deg, rgba(15,23,42,0.95) 0%, rgba(10,14,23,0.9) 100%)' }}>
      <div className="flex items-center justify-between max-w-[1280px] mx-auto px-3 sm:px-6 py-3">
        ...
        <nav className="hidden md:flex gap-1 items-center">
          {links.map(link => (
            <a key={link.href} href={link.href} className="px-3.5 py-1.5 text-[13px] font-medium text-white/55 hover:text-white/90 hover:bg-white/5 rounded-md transition-all">
              {link.label}
            </a>
          ))}
          <a href="https://github.com/ipalumbo73/hormuz-index" target="_blank" rel="noopener noreferrer" className="ml-1 px-2 py-1.5 text-white/30 hover:text-white/60 hover:bg-white/5 rounded-md transition-all" title="GitHub" aria-label="GitHub repository">
